...
elseif  strcmp(dataset,'Xenograft')
    ...
    filter_no_survival_pat = false;
    min_mutation = 10;
elseif  strcmp(dataset,'CNA')
    survival_dir = '../data/survival/CNA/';
    filter_no_survival_pat = false;
    min_mutation = 10;
else
    error('wrong dataset');
...
for fi = 1:length(files)
    ...
    nnedge = length(find(adj_network(:)>0));
    fprintf('cancer_type=%s finished compute adjnetwork,nedge:%d\n',cancer_type,nnedge);
  ...
  fprintf('cancer_type=%s finished compute ME network,nedge:%d\n',cancer_type,nedge);
